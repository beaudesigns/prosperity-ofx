...
var ofxToXML = exports.ofxToXML = function(ofx, callback) {
  // TODO (EK): Check for closing tags if so just return the XML

  // Try to parse as valid XML first.
  parser.parseString(ofx, function (error, object) {
    ...
    // If it's parsable XML, return the original string.
    return callback(ofx);
  });
};
...
exports.parse = function(data, callback) {
  ...
  ofxToXML(ofx, function (cleanXMLString) {
    parser.parseString(ofx, function (error, object) {
      if (error) {
